...
const getPresignedUrl = (file: string) => {
  return axios
    .get(`${file}?format=json`, {
      headers: { Authorization: createAuthHeaderValueForToken(keycloak.token) },
    })
    .then((response) => {
...
const generateCramTrack = (
  ...
  return {
    type: 'alignment',
    format: 'cram',
    url: getPresignedUrl(cramCraiFiles.mainFile!),
    indexURL: getPresignedUrl(cramCraiFiles.indexFile!),
    name: cramTrackName,
    height: 500,
    colorBy: 'strand',
    sort: {
...
const buildTracks = (
  ...
  if (donor.mother_id && motherFiles) {
    tracks.push(generateCramTrack(motherFiles, donor.mother_id, GENDER.FEMALE, PARENT_TYPE.MOTHER));
  }

  if (donor.father_id && fatherFiles) {
    tracks.push(generateCramTrack(fatherFiles, donor.father_id, GENDER.MALE, PARENT_TYPE.FATHER));
  }

  return tracks;
};

const IGVModal = ({ donor, variantEntity, isOpen = false, toggleModal }: OwnProps) => {
        ...
        !(loading || motherLoading || fatherLoading) && (
          <Igv
            ...
            options={{
              ...
              tracks: buildTracks(results!, motherResults, fatherResults, donor!),
            }}
          />
        )
      )}
